...
def build_transforms(train: bool = True, image_size: int = 384, use_advanced: bool = False):
    if train:
        if use_advanced:
            return A.Compose(
                [
...
def build_tta_transforms(image_size: int = 384) -> list:
...
def mixup_data(images: torch.Tensor, labels: torch.Tensor, alpha: float = 0.4) -> tuple:
...
def cutmix_data(images: torch.Tensor, labels: torch.Tensor, alpha: float = 1.0) -> tuple:
